...
C나 Objective-C의 할당 연산자와 다르게 Swift의 할당 연산자는 그 자체가 값을 반환하지 않습니다. 즉, 다음 문장은 유효하지 않습니다.
...
비교 연산자는 `if`문 등의 조건문에서 주로 사용됩니다.
...
`if`문에 대한 자세한 설명은 [제어 흐름](control-flow.md)을 참고하세요.
...
닫힌 범위 연산자는 `for`-`in`문에서와 같이 모든 값에 대해 범위를 만들어 반복하고자 할 때 유용합니다.
...
`for`-`in`문에 대해서는 [제어 흐름](control-flow.md)을 참고하세요.
...
이 예시에서처럼 불 상수나 변수의 이름을 적절히 고르면 코드의 가독성과 간결성이 증가하고 이중 부정이나 헷갈리는 논리문을 피할 수 있습니다.
