...
Swift의 모든 기초 자료형(`String`, `Int`, `Double`, `Bool` 등)은 기본적으로 해싱이 가능합니다. 이들이 집합의 값이나 딕셔너리의 키에 사용될 수 있습니다. 연관 값이 없는 열거형 값([열거형](enumerations.md)에서 설명)들도 기본으로 해싱이 가능합니다.
